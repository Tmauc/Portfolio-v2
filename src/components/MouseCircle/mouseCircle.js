...
import { CursorInner, CursorOuter } from './mouseCircle.style'
...
function AnimatedCursor() {
  ...
  const isElementClickable = (element) => {
    if (['svg', 'path', 'iframe'].includes(element.tagName)) return
    if (element.className) {
      return element?.className?.includes('clickable');
    }
  }

  // Event handlers for mouseover/mouseout
  ...
  const scaleBySize = useCallback(
    (cursorRef, orignalSize, scaleAmount) => {
      if (cursorRef) {
